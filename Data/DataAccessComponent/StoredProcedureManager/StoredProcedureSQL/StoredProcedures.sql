...
BEGIN
    ...
    -- Begin Insert Statement
    Insert Into [Stock]
    ([AverageDailyVolume],[Country],[DaysBelowMinVolume],[Exchange],[Industry],[IPOYear],[LastClose],[LastUpdated],[Name],[Sector],[Streak],[Symbol],[Track])

    -- Begin Values List
    Values(@AverageDailyVolume, @Country, @DaysBelowMinVolume, @Exchange, @Industry, @IPOYear, @LastClose, @LastUpdated, @Name, @Sector, @Streak, @Symbol, @Track)
...
BEGIN
    ...
    -- Begin Select Statement
    Select [AverageDailyVolume],[Country],[DaysBelowMinVolume],[Exchange],[Id],[Industry],[IPOYear],[LastClose],[LastUpdated],[Name],[Sector],[Streak],[Symbol],[Track]
...
Go
-- =========================================================
-- Procure Name: DailyPriceData_FindByStockDateAndSymbol
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   4/29/2025
...
-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('DailyPriceData_FindByStockDateAndSymbol'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure DailyPriceData_FindByStockDateAndSymbol

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.DailyPriceData_FindByStockDateAndSymbol') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure DailyPriceData_FindByStockDateAndSymbol >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure DailyPriceData_FindByStockDateAndSymbol >>>'

    End

GO

Create PROCEDURE DailyPriceData_FindByStockDateAndSymbol
...
Go
-- =========================================================
-- Procure Name: DailyPriceData_FindMaxStreakBySymbol
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   4/29/2025
-- Description:    Find an existing DailyPriceData for the Symbol given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('DailyPriceData_FindMaxStreakBySymbol'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure DailyPriceData_FindMaxStreakBySymbol

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.DailyPriceData_FindMaxStreakBySymbol') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure DailyPriceData_FindMaxStreakBySymbol >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure DailyPriceData_FindMaxStreakBySymbol >>>'

    End

GO

Create PROCEDURE DailyPriceData_FindMaxStreakBySymbol
...
Go
-- =========================================================
-- Procure Name: DoNotTrack_FindBySymbol
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   4/29/2025
-- Description:    Find an existing DoNotTrack for the Symbol given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('DoNotTrack_FindBySymbol'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure DoNotTrack_FindBySymbol

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.DoNotTrack_FindBySymbol') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure DoNotTrack_FindBySymbol >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure DoNotTrack_FindBySymbol >>>'

    End

GO

Create PROCEDURE DoNotTrack_FindBySymbol
