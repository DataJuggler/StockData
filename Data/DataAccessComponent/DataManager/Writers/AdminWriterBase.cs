    ...
    public class AdminWriterBase
    {
        ...
        #region Static Methods
            ...
            internal static SqlParameter[] CreateInsertParameters(Admin admin)
            {
                ...
                if(admin != null)
                {
                    // Create [DocumentsFolder] parameter
                    param = new SqlParameter("@DocumentsFolder", admin.DocumentsFolder);

                    // set parameters[0]
                    parameters[0] = param;

                    // Create [MinVolume] parameter
                    param = new SqlParameter("@MinVolume", admin.MinVolume);

                    // set parameters[1]
                    parameters[1] = param;

                    // Create [ProcessedFolder] parameter
                    param = new SqlParameter("@ProcessedFolder", admin.ProcessedFolder);
            ...
            internal static SqlParameter[] CreateUpdateParameters(Admin admin)
            {
                ...
                if(admin != null)
                {
                    // Create parameter for [DocumentsFolder]
                    param = new SqlParameter("@DocumentsFolder", admin.DocumentsFolder);

                    // set parameters[0]
                    parameters[0] = param;

                    // Create parameter for [MinVolume]
                    param = new SqlParameter("@MinVolume", admin.MinVolume);

                    // set parameters[1]
                    parameters[1] = param;

                    // Create parameter for [ProcessedFolder]
                    param = new SqlParameter("@ProcessedFolder", admin.ProcessedFolder);
